...
plot(EN.trn, xvar = "lambda")

CV.Elastic = cv.glmnet(SM.trn.x, SM.trn.y, family = "binomial", alpha = 0.5, nfolds = 5)
plot(CV.Elastic)
best_lambda = CV.Elastic$lambda.min

EN.coeff = predict(EN.trn, s = best.lambda, newx = SM.val.x, type = "coefficient")

EN.yhat = predict(EN.trn, s = best.lambda, newx = SM.val.x, type = "class")
...
RF.trn = randomForest(target ~ .,df.trn, ntree = 200)
summary(RF.trn)
